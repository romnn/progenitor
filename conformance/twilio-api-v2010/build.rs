//! Generate the Twilio API v2010 client at build time from the
//! corpus-cached spec (downloaded on first use by the wild-tests fetch
//! helper).
...
fn main() {
    let entry = wild_tests::load_manifest()
        ...
        .into_iter()
        .find(|entry| entry.name == "twilio-api-v2010")
        .expect("twilio-api-v2010 is in the corpus manifest");
    println!(
