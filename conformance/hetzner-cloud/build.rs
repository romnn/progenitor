//! Generate the Hetzner Cloud client at build time from the corpus-cached
...
fn main() {
    let entry = wild_tests::load_manifest()
        ...
        .into_iter()
        .find(|entry| entry.name == "hetzner-cloud")
        .expect("hetzner-cloud is in the corpus manifest");
    println!(
