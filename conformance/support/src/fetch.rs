...
pub struct SpecManifest {
    ...
    /// Generate the server-stub module (`$OUT_DIR/server.rs`). Defaults to
    /// `true` so every spec in the corpus exercises the server generator; set
    /// `server = false` to opt a crate out.
    #[serde(default = "default_true")]
    pub server: bool,
    /// Generate the httpmock helper module (`$OUT_DIR/mock.rs`). Defaults to
    /// `false`: generating it costs a second full `Generator::prepare` pass and
    /// only the crates whose tests `include!` mock.rs have any use for it.
    #[serde(default)]
    pub mock: bool,
    /// Embed each type's full JSON schema in its doc comment
    /// ([`progenitor_impl::SchemaDocs::Full`]). Defaults to `false`: those
    /// blocks are roughly half the corpus's generated bytes and nothing here
    /// renders rustdoc. petstore-31 keeps them on so the doc-emission path
    /// stays covered by a compiled crate.
    #[serde(default)]
    pub schema_docs: bool,
}
