...
impl TypeSpace {
    ...
    pub(crate) fn maybe_internally_tagged_enum(
        ...
        // required, *fixed-value* property in common. Resolve any
        // `$ref`-only subschema against the type space's known
        // definitions before pattern-matching so a `oneOf: [{$ref: …}, ...]`
        // (the common shape an OpenAPI `discriminator + allOf` group
        // emits after the discriminator pre-pass, but also a perfectly
        // plain JSON-Schema-with-refs `oneOf`) can correctly dispatch
        // to internally-tagged when the resolved subtypes share a
        // common const-valued tag property. Without this resolution,
        // typify can't see through the ref and falls through to
...
mod tests {
    ...
    /// Regression: a `oneOf` over `$ref`-only subschemas whose resolved
    /// targets share a required const-valued property gets emitted as
    /// `#[serde(tag = "<property>")]`. Prior to this fix typify could
    /// not see through the refs in `maybe_internally_tagged_enum`, so
    /// it fell through to the untagged path — which then deserialised
    /// by structural shape and picked the wrong variant when subtype
    /// payloads overlap. The test was previously named
    /// `test_tricky_untagged_enum` and asserted `Untagged`; both the
    /// name and the assertion reflected the prior limitation, not the
    /// semantically right answer.
    #[test]
    fn test_ref_only_oneof_with_const_tag_becomes_internally_tagged() {
        let schema_json = r##"
        {
            "definitions": {
                "workflow-step-completed": {
