...
/// Map a single character of a symbol-only enum value (e.g. "<=" or "~") to a
/// readable identifier fragment. Returns None for characters that have no
/// spelled-out form; `spelled_out_name` only succeeds when every character of
/// the value maps, so values containing "normal" identifier characters are
/// left to the usual sanitization passes.
fn symbol_char_word(c: char) -> Option<&'static str> {
    let word = match c {
        '=' => "Eq",
        '!' => "Not",
        '<' => "Lt",
...
/// Compute a readable variant name for enum values that consist purely of
/// punctuation or whitespace ("<", ">=", "~=", " ", ...). Sanitization elides
/// such characters entirely, so operator-style values otherwise collapse into
/// identical (or empty) identifiers. Returns None if the value is empty or
/// contains any character without a spelled-out form.
fn spelled_out_name(raw_name: &str) -> Option<String> {
    if raw_name.is_empty() {
        return None;
    }
    raw_name.chars().map(symbol_char_word).collect()
}
...
impl TypeEntryEnum {
    pub(crate) fn from_metadata(
        ...
        // Wild schemas sometimes repeat the same value within an `enum` array
        // (e.g. ["webhook", "integration", "webhook"]). Such duplicates carry
        // no information, would produce irreconcilable variant-name
        // collisions below, and would emit duplicate serde renames; keep the
        // first occurrence of each.
        let mut seen = BTreeSet::new();
        variants.retain(|variant| seen.insert((variant.raw_name.clone(), variant.details.clone())));
        ...
        // If variants still aren't unique--wild schemas contain values that
        // differ only by case ("ASC"/"asc") and values made purely of
        // punctuation ("<", ">=") that all sanitize to the same
        // identifier--fall back to a deterministic scheme rather than
        // panicking: values composed entirely of symbols or whitespace are
        // spelled out ("<=" becomes "LtEq") and any name that still collides
        // gets a numeric suffix in declaration order ("ASC"/"asc" become
        // "Asc"/"Asc2"). The serde rename always carries the original raw
        // value, so serialization is unaffected.
        if !variants_unique(&variants) {
            let mut used = BTreeSet::new();
            variants.iter_mut().for_each(|variant| {
                let base = spelled_out_name(&variant.raw_name)
                    .unwrap_or_else(|| variant.ident_name.clone().unwrap());
                let mut ident_name = base.clone();
                let mut suffix = 2u32;
                while !used.insert(ident_name.clone()) {
                    ident_name = format!("{}{}", base, suffix);
                    suffix += 1;
                }
                variant.ident_name = Some(ident_name);
            });
...
impl TypeEntry {
    ...
    /// Overwrite this type's emitted Rust identifier. Used by `TypeSpace`
    /// when a ref-derived component schema arrives wanting a name that an
    /// inline schema already claimed via property-based inference — the
    /// inline entry gets renamed so the explicitly-named component wins.
    /// No-op for variants without a name field (`Reference`, `Native`, ...).
...
mod tests {
    ...
    /// Build a string enum from the given values and return the generated
    /// variant identifiers along with the emitted output (for checking serde
    /// renames).
    fn enum_variants(values: serde_json::Value) -> (Vec<String>, String) {
        ...
        let variants = file
            ...
            .find_map(|item| match item {
                syn::Item::Enum(item_enum) if item_enum.ident == "TestEnum" => Some(
                    item_enum
                        .variants
                        .iter()
            ...
            .expect("enum was generated");
        (variants, file.to_token_stream().to_string())
    }
    ...
    #[test]
    fn test_enum_variants_case_collision() {
        // Values that differ only by case sanitize to the same Pascal-case
        // identifier; later occurrences get a numeric suffix.
        let (variants, output) = enum_variants(serde_json::json!(["ASC", "DESC", "asc", "desc"]));
        assert_eq!(variants, vec!["Asc", "Desc", "Asc2", "Desc2"]);
        assert!(output.contains(r#"rename = "ASC""#));
        assert!(output.contains(r#"rename = "asc""#));
        assert!(output.contains(r#"rename = "DESC""#));
        assert!(output.contains(r#"rename = "desc""#));

        let (variants, output) = enum_variants(serde_json::json!(["and", "AND", "or", "OR"]));
        assert_eq!(variants, vec!["And", "And2", "Or", "Or2"]);
        assert!(output.contains(r#"rename = "AND""#));
        assert!(output.contains(r#"rename = "OR""#));

        let (variants, output) = enum_variants(serde_json::json!(["ELIGIBLE", "Eligible"]));
        assert_eq!(variants, vec!["Eligible", "Eligible2"]);
        assert!(output.contains(r#"rename = "ELIGIBLE""#));
        assert!(output.contains(r#"rename = "Eligible""#));
    }

    #[test]
    fn test_enum_variants_symbols() {
        // Operator values consist entirely of punctuation, all of which is
        // elided by sanitization; they get spelled-out names instead.
        let (variants, output) = enum_variants(serde_json::json!([
            "=", "!=", ">", "<", ">=", "<=", "~", "~="
        ]));
        assert_eq!(
            variants,
            vec!["Eq", "NotEq", "Gt", "Lt", "GtEq", "LtEq", "Tilde", "TildeEq"]
        );
        ...
        let (variants, output) = enum_variants(serde_json::json!(["<", ">"]));
        assert_eq!(variants, vec!["Lt", "Gt"]);
        assert!(output.contains(r#"rename = "<""#));
        assert!(output.contains(r#"rename = ">""#));
    }

    #[test]
    fn test_enum_variants_duplicate_values() {
        // Literal duplicate values in the enum array are a spec bug; the
        ...
        assert_eq!(variants, vec!["GitProvider", "Integration", "Webhook"]);
        assert!(output.contains(r#"rename = "git-provider""#));
    }

    #[test]
    fn test_enum_variants_whitespace_and_empty() {
        // A lone space spells out to "Space"; the empty string falls back to
        // the sanitization placeholder.
        let (variants, output) = enum_variants(serde_json::json!([" ", ""]));
        assert_eq!(variants, vec!["Space", "X"]);
        assert!(output.contains(r#"rename = " ""#));
        assert!(output.contains(r#"rename = """#));
    }
}
