...
impl TypeEntry {
    pub(crate) fn check_defaults(&mut self, type_space: &mut TypeSpace) -> Result<()> {
        // Check the "whole-type" default.
        let whole_default = match &self.details {
        ...
        if let Some(default) = whole_default {
            match self.validate_value(type_space, &default) {
                ...
                // data--not in typify--so rather than failing the whole
                // TypeSpace we generate the type as if no default were
                // present. Programmer errors (any other error variant) still
                // propagate.
                Err(Error::InvalidValue) => match &mut self.details {
                    TypeEntryDetails::Enum(TypeEntryEnum { default, .. })
                    | TypeEntryDetails::Struct(TypeEntryStruct { default, .. })
                    | TypeEntryDetails::Newtype(TypeEntryNewtype { default, .. }) => {
                        *default = None;
                    }
                    _ => unreachable!(),
                },
        ...
        match &mut self.details {
            ...
            TypeEntryDetails::Enum(TypeEntryEnum { variants, .. }) => {
                variants.iter_mut().try_for_each(|variant| {
                    if let VariantDetails::Struct(properties) = &mut variant.details {
                        properties
                            .iter_mut()
                            .try_for_each(|prop| Self::check_property_defaults(prop, type_space))
                    } else {
    ...
    fn check_property_defaults(
        property: &mut StructProperty,
        type_space: &mut TypeSpace,
    ) -> Result<()> {
        let (result, has_intrinsic_default) = match &*property {
            StructProperty {
                ...
                let has_intrinsic_default = matches!(
                    &type_entry.details,
        ...
        match result {
            ...
            // (e.g. `default: null` on a non-nullable string). Drop the
            // bogus default and treat the property exactly as if no default
            // were given: types with an intrinsic default keep their
            // `#[serde(default)]` handling; everything else becomes
            // `Option<T>` (mirroring the no-default path in
            // `struct_property`). Programmer errors (any other error
            // variant) still propagate.
            Err(Error::InvalidValue) => {
                if !has_intrinsic_default {
                    property.type_id = type_space.id_to_option(&property.type_id);
                }
                property.state = StructPropertyState::Optional;
                Ok(())
            }
            Err(other) => Err(other),
        }
    ...
    pub(crate) fn validate_value(
        ...
        match &self.details {
            ...
            // Wild specs carry defaults the type can't represent (e.g.
            // `default: -1` on a uint); the conversion routines no longer
            // reject those, so an out-of-range default must be flagged here
            // as nonconforming--callers respond by dropping it--lest we emit
            // a default fn whose `try_from(...).unwrap()` panics at runtime.
            ...
            TypeEntryDetails::String => match default.as_str() {
                Some("") => Ok(DefaultKind::Intrinsic),
                Some(_) => Ok(DefaultKind::Specific),
                // A non-string default (e.g. `null`) on a string type is a
                // schema-data flaw; report it as such so callers can ignore
                // the default rather than failing at code-emission time.
                None => Err(Error::invalid_value()),
            },

            // References should all be resolved by the time defaults are
            // validated, but wild schemas have demonstrated paths where an
            // unresolved reference entry survives this long. The default
            // can't be checked against a type we can't see, so report it as
            // non-conforming--callers respond by dropping the default--rather
            // than aborting code generation with a panic.
...
/// Determine whether an integer default can be represented by the named Rust
/// integer type. Non-numeric values are out of range by definition; type
/// names we don't recognize are left unconstrained.
fn integer_in_range(itype: &str, default: &serde_json::Value) -> bool {
    let value: i128 = if let Some(value) = default.as_u64() {
        value.into()
    } else if let Some(value) = default.as_i64() {
        value.into()
    } else {
    ...
    let (min, max): (i128, i128) = match itype.strip_prefix(STD_NUM_NONZERO_PREFIX) {
        Some("U8") => (1, u8::MAX.into()),
        Some("U16") => (1, u16::MAX.into()),
        Some("U32") => (1, u32::MAX.into()),
        Some("U64") => (1, u64::MAX.into()),
        _ => match itype {
...
mod tests {
    ...
    fn test_default_numbers_and_string() {
        ...
        // Non-string defaults on a string type don't conform.
        assert!(type_entry
            .validate_value(&type_space, &json!(null))
            .is_err());
        assert!(type_entry.validate_value(&type_space, &json!(42)).is_err());
    }

    #[test]
    fn test_default_on_unresolved_reference() {
        let (type_space, type_id) = get_type::<u32>();

        let type_entry = TypeEntry {
            details: crate::type_entry::TypeEntryDetails::Reference(type_id),
            extra_derives: Default::default(),
            extra_attrs: Default::default(),
        };

        // A default attached to a still-unresolved reference can't be
        // checked against anything; it must be reported as non-conforming
        // (so callers drop the default) rather than panicking.
        assert!(type_entry.validate_value(&type_space, &json!(42)).is_err());
        assert!(type_entry
            .validate_value(&type_space, &json!(null))
            .is_err());
    ...
    #[test]
    fn test_bogus_property_default_ignored() {
        // Wild specs routinely carry property defaults that don't conform to
        // the property's schema; `default: null` on a non-nullable string is
        // the canonical case. The type must still generate--just without the
        // bogus default.
        let schema_json = r#"
        {
            "title": "Widget",
        ...
        // The property is emitted as if no default were present: an optional
        // string rather than a string with a bespoke default function.
        assert!(
            actual.contains("pub name : :: std :: option :: Option < :: std :: string :: String >")
        );
        assert!(!actual.contains("widget_name"));
    }

    #[test]
    fn test_bogus_whole_type_default_ignored() {
        // A type-level default that doesn't conform to the type's own schema
        // (here: a value absent from the enumerated set) is dropped rather
        // than failing the whole TypeSpace.
        let schema_json = r#"
        {
            "title": "Sort",
            "type": "string",
        ...
        let actual = type_space.to_stream().to_string();
        assert!(actual.contains("enum Sort"));
        // No Default impl is generated for the nonconforming default.
        assert!(!actual.contains("impl :: std :: default :: Default for Sort"));
    }
