//! The `types::de` runtime, emitted verbatim into each generated crate that
//! uses [`DeserializeImpl::Buffered`](crate::DeserializeImpl::Buffered).
//!
//! Generated `Deserialize` impls are two parts: a tiny generic `deserialize<D>`
//! that drives `D::deserialize_struct` with the shared [`StructVisitor`] below,
//! and a fully monomorphic `__build` that reads the buffered fields. The point
//! is that only the second part is per-type, and it is not generic over the
//! `Deserializer`, so rustc type-checks one small body per struct instead of
//! serde_derive's nine (mostly generic) ones.
//!
//! `serde`'s own equivalent lives in `serde::__private::de`, but that module's
//! name is suffixed with serde's patch version by its build script (`pub mod
//! __private$$`), so no generator can name it — hence this copy.
//!
//! This file is not compiled as part of `typify-impl`; it is `include_str!`d and
//! re-parsed into the generated output. Keeping it as ordinary Rust rather than
//! a `quote!` block means it can be edited, formatted and tested directly.
...
///
/// The map arm is an association list rather than a map so that duplicate keys
/// survive buffering: `serde_derive` rejects `{"a":1,"a":2}` and collapsing
...
impl<'de> Content<'de> {
    ...
    /// `Unexpected::Unit` so messages read `invalid type: null, expected …`
    ...
    /// The string form of a buffered map key, used to match field names.
...
impl<'de> Deserialize<'de> for Content<'de> {
    fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        // `deserialize_any` is required here, but only ever reached for a
        // *field value* — the struct itself is entered through
        // `deserialize_struct`, so self-describing-ness is only needed for the
        // values, exactly as it already is for `serde_json::Value` fields.
...
impl<'de, E: de::Error> Deserializer<'de> for ContentDeserializer<'de, E> {
    ...
    fn deserialize_any<V: Visitor<'de>>(self, visitor: V) -> Result<V::Value, E> {
        match self.content {
            ...
            // Replaying is not enough on its own: a fixed-arity visitor
            // (`[T; N]`, a tuple, a tuple struct) stops asking at its own
            // length, so without the `end` check the surplus elements would be
            // dropped in silence — input the format itself would have refused.
            Content::Seq(v) => {
        ...
        let (variant, value) = match self.content {
            Content::Map(entries) => {
                ...
                let (variant, value) = match iter.next() {
                    Some(kv) => kv,
                    None => {
                        return Err(E::invalid_value(Unexpected::Map, &"map with a single key"));
                    }
                };
    ...
    forward_number! {
        ...
        deserialize_string => visit_string,
        deserialize_bytes => visit_bytes,
        deserialize_byte_buf => visit_byte_buf,
        deserialize_unit => visit_unit,
        deserialize_seq => visit_seq,
        deserialize_map => visit_map,
        deserialize_identifier => visit_identifier,
        deserialize_ignored_any => visit_ignored_any,
    }
...
impl<'de, E: de::Error> SeqReplay<'de, E> {
    ...
    /// Reject whatever the visitor did not consume, in serde's own wording for
    /// a replayed sequence.
...
impl<'de, E: de::Error> MapReplay<'de, E> {
    ...
    /// The map counterpart of [`SeqReplay::end`]. Struct visitors drain the
    /// map, so this only bites for a visitor with a fixed shape — but the
...
/// `struct Foo with 3 elements`, distinct from the visitor's own `expecting`.
struct StructLen {
    name: &'static str,
    len: usize,
}
...
/// Visitor a generated `deserialize` hands to `Deserializer::deserialize_struct`.
///
/// Implements both `visit_map` and `visit_seq` so non-self-describing formats
/// (bincode, postcard, MessagePack in compact mode) keep working: a sequence is
/// zipped against the struct's static field list here, in shared code, so
/// `__build` only ever sees a map and pays nothing per struct for it.
/// Per-type half of a buffered `Deserialize`, implemented by generated types.
///
/// Splitting it out this way is what keeps the emitted code to two bodies per
/// struct: everything that walks the input lives in the generic
/// [`StructVisitor`] below and is type-checked once, no matter how many types
/// implement this.
...
/// Drives `T`'s buffered deserialization. The whole per-type `deserialize`.
...
///
/// Implements both `visit_map` and `visit_seq` so non-self-describing formats
/// keep working: a sequence is zipped against the struct's static field list
/// here, in shared code, so `build` only ever sees a map.
///
/// `build` is called *inside* the visitor rather than after it returns, which
/// is what keeps a format's positional error fixup (serde_json's line/column)
    ...
    fn visit_seq<A: SeqAccess<'de>>(self, mut seq: A) -> Result<Self::Value, A::Error> {
        ...
        for (index, name) in T::FIELDS.iter().enumerate() {
            match seq.next_element::<Content<'de>>()? {
                ...
                // Matches the derive, which reports a short sequence as a
                // length error rather than letting it surface later as a
                // missing field.
                None => {
...
/// Take the value for `name`, erroring on a duplicate the way the derive does.
///
/// Returns the raw `Content` so the caller decides between "absent" and
/// "present but null", which `serde_derive` distinguishes and a `HashMap`-based
/// buffer cannot.
...
///
/// Unlike [`Build`], this involves **no buffering**: a unit variant is decided
/// from the identifier alone, so the generated impl drives `deserialize_enum`
/// directly and keeps working with non-self-describing formats, and any error
...
pub trait UnitEnum: Sized {
    ...
    ///
    /// Only ever called with an index this module has already bounds-checked
    /// against `VARIANTS`.
...
impl<'de, T: UnitEnum> Visitor<'de> for VariantIndexVisitor<T> {
    ...
    // Self-describing formats name the variant; compact ones send its index.
    // Both spellings are what `serde_derive` accepts, including the exact
    // wording of the two rejections.
    fn visit_u64<E: de::Error>(self, value: u64) -> Result<Self::Value, E> {
        // Narrowing through `try_from` rather than `as`: on a 32-bit target an
        // out-of-range index would otherwise wrap into a valid one.
...
///
/// An absent key is a `missing field` error for most types — but *not* for
/// `Option<T>`, which sees `None`. serde gets this by asking `T` to deserialize
/// from a source that answers `visit_none` to `deserialize_option` and errors
/// for everything else, so the special case lives in `Option`'s own impl rather
/// than in the generated code. A by-name lookup has to do the same, or it
/// rejects documents the derive accepts whenever a schema marks a nullable
/// property required.
...
/// Deserialize a field that falls back to `Default` when the key is absent.
///
/// Mirrors `#[serde(default)]`: an absent key uses the default, while a present
/// `null` is still handed to `T`'s own impl (so `Option<T>` sees `None` and a
...
/// Deserialize a field whose absent-value comes from a named function, as
