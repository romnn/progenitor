...
fn try_merge_schema(a: &Schema, b: &Schema, defs: &BTreeMap<RefKey, Schema>) -> Result<Schema, ()> {
    match (a, b) {
        ...
        | (
            ...
            // Wild specs (Square) carry $refs to schemas that were never
            // registered. There is no data we can prove satisfies a schema
            // we cannot see, so treat the merge as unsatisfiable--callers
            // degrade to the never type--rather than panicking and taking
            // down the whole generation.
            let resolved = defs.get(&key).ok_or(())?;
...
pub(crate) fn try_merge_with_subschemas(
    ...
    // A schema may carry both anyOf and oneOf (JSON Schema requires that
    // both hold; ElevenLabs' OpenAPI spec does this in the wild). The
    // sequential handling below already implements exactly that semantic:
    // the anyOf pass folds the schema body into the anyOf union, and the
    // oneOf pass then merges that union schema against each oneOf
    // alternative--i.e. the result is the allOf of the two union groups.
...
mod tests {
    ...
    #[test]
    fn test_any_of_and_one_of_together() {
        // JSON Schema allows anyOf and oneOf side by side (both must hold);
        // ElevenLabs' OpenAPI spec does this in the wild. The merge is the
        // allOf of the two union groups and must not panic.
        let schema_json = r#"
        {
