...
/// Error for a `$ref` that names a schema absent from the definitions the
/// caller registered--incomplete input rather than an internal failure.
fn missing_ref_error(ref_name: &str) -> Error {
    Error::InvalidSchema {
        type_name: None,
        reason: format!("$ref {} is missing from the definitions", ref_name),
    }
}

/// Combine the metadata of an outer schema with metadata that survived a
/// schema merge. Where both carry a value for the same field, the outer
/// schema--the one nearer the point of use--wins.
fn merge_metadata(
    outer: &Option<Box<Metadata>>,
    inner: Option<Box<Metadata>>,
) -> Option<Box<Metadata>> {
    match (outer.clone(), inner) {
        (None, metadata) | (metadata, None) => metadata,
        (Some(outer), Some(inner)) => Some(Box::new(Metadata {
            id: outer.id.or(inner.id),
            title: outer.title.or(inner.title),
            description: outer.description.or(inner.description),
            default: outer.default.or(inner.default),
            deprecated: outer.deprecated || inner.deprecated,
            read_only: outer.read_only || inner.read_only,
            write_only: outer.write_only || inner.write_only,
            examples: if outer.examples.is_empty() {
                inner.examples
            } else {
                outer.examples
            },
...
impl TypeSpace {
    ...
    pub(crate) fn convert_schema_object<'a>(
        ...
        match schema {
            ...
            SchemaObject {
                ...
                // A missing definition falls through to convert_reference,
                // which reports the dangling $ref as an error.
                if let Some(ref_schema) = self.definitions.get(&ref_key(reference)) {
            ...
            SchemaObject {
                ...
                let mut def = self
                    ...
                    .ok_or_else(|| missing_ref_error(reference))?;
                ...
                while let Schema::Object(SchemaObject { reference: r, .. }) = def {
                    ...
                    if let Some(r) = r {
                        def = self
                            .definitions
                            .get(&ref_key(r))
                            .ok_or_else(|| missing_ref_error(r))?;
                    } else {
                        break;
                    }
                }
                let (type_entry, _) = self.convert_schema(type_name, &new_schema)?;
            ...
            // A bare `{ "format": "<something>" }` with no `type` shows up
            // in real-world specs (especially when the author meant
            // "a string formatted as email/uri/uuid/…" and relied on the
            // reader to infer `type: string` from the format keyword).
            // Route through the string converter as if the implicit
            // `type: string` had been declared.
            SchemaObject {
                format,
                instance_type: None,
                string,
                ..
            } if format.is_some() => self.convert_string(
                type_name,
                original_schema,
                &schema.metadata,
                format,
            ...
            // Wild specs (Twilio, Cloudflare, Box) attach `format` values to
            // types where typify has no special handling, e.g.
            // `{"type": "object", "format": "uri-map"}`. A format that no
            // earlier arm recognized is annotation rather than constraint:
            // drop it and convert as though it were absent. The arms above
            // handle every format typify understands (and types like integer
            // and number tolerate unknown formats themselves), so anything
            // arriving here would otherwise fall into the todo!() below.
            SchemaObject {
                metadata,
                format: Some(_),
                ..
    ...
    pub(crate) fn convert_enum_string<'a>(
        ...
        let variants = enum_values
            ...
            .flat_map(|value| match value {
                ...
                // Wild specs mix values of other types (numbers, booleans)
                // into string enums. Such values can never satisfy the
                // string type so--like the constraint-violating strings
                // above--they're unsatisfiable variants; skip them rather
                // than aborting generation of the whole document.
                _ => None,
            })
    ...
    fn convert_integer<'a>(
        ...
        if let Some(format) = format {
                ...
                .find(|(int_format, _, _, _, _)| int_format == format)
            {
                ...
                if multiple.is_none() && valid_min && valid_max {
                    // Note that we don't check the default value against the
                    // format's range here. Wild specs (OpenAI, Discord,
                    // ClickHouse, AssemblyAI, HF-TGI, Linode, Plaid) carry
                    // defaults like `default: -1` on a uint; the type
                    // converts as if no default were present and the
                    // defaults check later drops any default the chosen Rust
        ...
        // We intentionally don't reject a default value that violates the
        // schema's min/max bounds: wild specs routinely carry such defaults
        // and a conversion-time error would abort the whole document. The
        // defaults check validates the default against the Rust type chosen
        // below and drops it if it can't be represented.
    ...
    fn convert_reference<'a>(
        ...
        if !ref_name.starts_with('#') {
            return Err(Error::InvalidSchema {
                type_name: None,
                reason: format!("external references are not supported: {}", ref_name),
            });
        }
        let key = ref_key(ref_name);
        // Wild specs (Square) contain $refs that name schemas absent from
        // the document; every definition handed to typify is registered in
        // ref_to_id, so a missing entry means the input was incomplete.
        // Report it as an error rather than panicking so callers can
        // surface the dangling reference.
        let type_id = self
            .ref_to_id
            .get(&key)
            .ok_or_else(|| missing_ref_error(ref_name))?;
        Ok((
    ...
    fn convert_all_of<'a>(
        ...
        if let Schema::Bool(false) = &merged_schema {
            ...
            // The merged schema can arrive here carrying metadata: merging
            // `true` with a schema--or a `$ref` with another schema it
            // subsumes--returns a clone of the surviving schema, metadata
            // included (Meilisearch reaches this through $ref-with-sibling-
            // description chains). Fold that metadata together with the
            // outer schema's rather than asserting it away; outer values
            // take precedence.
            merged_schema.metadata = merge_metadata(metadata, merged_schema.metadata.take());
    ...
    fn convert_unknown_enum<'a>(
        ...
        fn implied_instance_type(value: &serde_json::Value) -> InstanceType {
            match value {
        ...
        if has_null {
            ...
            match (instance_types.len(), instance_types.iter().next()) {
                ...
                // Wild specs (Vercel) enumerate values of several JSON types
                // in one untyped enum, e.g. [false, "maybe", true]. No single
                // Rust type holds such an enumeration, so we degrade to an
                // untagged union over the implied types: each implied type
                // becomes a variant--named for the type via the title/allOf
                // wrapping, as in the multi-type instance handling--whose
                // schema keeps the enumerated values of that type. Booleans
                // drop their value constraint, mirroring the single-type
                // boolean case above.
                _ => {
                    let subschemas = instance_types
                        ...
                        .map(|instance_type| {
                            let values = enum_values
                                .iter()
                                .filter(|value| implied_instance_type(value) == *instance_type)
                                .cloned()
                                .collect::<Vec<_>>();

                            let (label, enum_values) = match instance_type {
                                InstanceType::Boolean => ("boolean", None),
                                InstanceType::Number => ("number", Some(values)),
                                InstanceType::String => ("string", Some(values)),
                                InstanceType::Array => ("array", Some(values)),
                                InstanceType::Object => ("object", Some(values)),
                                InstanceType::Null | InstanceType::Integer => unreachable!(),
                            };
                            let inner_schema = SchemaObject {
...
mod tests {
    ...
    #[test]
    fn test_low_default() {
        // A default below the type's range (`default: -1` on a uint; seen
        // in OpenAI, Discord, ClickHouse, AssemblyAI, HF-TGI, Linode, and
        // Plaid specs) no longer aborts conversion: the type converts as if
        // no default were present.
        ...
        let (ty, _) = type_space
            ...
            .unwrap();
        assert_eq!(ty.type_name(&type_space), "u32");

        // Used as a property, the nonconforming default is dropped during
        // the defaults check: the property reverts to the no-default path
        // (Option<u32>) and no default fn is emitted.
        let schema_json = r#"
        {
            "title": "LowDefault",
            "type": "object",
            "properties": {
                "count": { "type": "integer", "format": "uint", "default": -1 }
            }
        }
        "#;
        ...
        assert!(actual.contains("Option < u32 >"), "{}", actual);
        assert!(!actual.contains("default_u64"), "{}", actual);
    ...
    fn test_high_default() {
        ...
        // type's range so the type falls back to i64; the default is then
        // representable and survives--typify doesn't encode numeric bounds
        // into the generated type.
        let schema = SchemaObject {
            instance_type: Some(InstanceType::Integer.into()),
            metadata: Some(
                Metadata {
        ...
        let (ty, _) = type_space
            ...
            .unwrap();
        assert_eq!(ty.type_name(&type_space), "i64");
    }

    #[test]
    fn test_string_enum_with_wrong_typed_values() {
        // Wild specs mix non-string values into string enums. Those values
        // can never satisfy the declared type, so they're unsatisfiable
        // variants to be skipped--not grounds for aborting the document.
        let schema_json = r#"
        {
            "title": "Status",
            "type": "string",
            "enum": ["ok", 42, true, "failed"]
        }
        "#;
        ...
        let _ = type_space.add_type(&schema.schema.into()).unwrap();
        let actual = type_space.to_stream().to_string();
        assert!(actual.contains("enum Status"), "{}", actual);
        assert!(actual.contains("Ok"), "{}", actual);
        assert!(actual.contains("Failed"), "{}", actual);
    }

    #[test]
    fn test_mixed_type_untyped_enum() {
        // Vercel's spec contains untyped enums whose values mix JSON types.
        // Rather than panicking ("multiple implied types for an un-typed
        // enum"), we degrade to an untagged union over the implied types,
        // keeping per-type value constraints where possible (the string
        // variant here; booleans drop their constraint just as single-type
        // boolean enums do).
        let schema_json = r#"
        {
            "title": "MaybeBool",
        ...
        let _ = type_space.add_type(&schema.schema.into()).unwrap();
        let actual = type_space.to_stream().to_string();
        assert!(actual.contains("enum MaybeBool"), "{}", actual);
    ...
    fn test_missing_ref_is_an_error() {
        ...
        // panic--so callers can report the dangling reference.
        let schema_json = r##"{ "$ref": "#/components/schemas/AppFeeAllocation" }"##;
        let schema: RootSchema = serde_json::from_str(schema_json).unwrap();
        let mut type_space = TypeSpace::default();
        match type_space.add_type(&schema.schema.into()) {
            Err(Error::InvalidSchema { reason, .. }) => {
                assert!(reason.contains("AppFeeAllocation"), "{}", reason);
            }
            Ok(_) => panic!("dangling $ref unexpectedly converted"),
            Err(other) => panic!("unexpected error: {}", other),
        }

        // The same dangling reference inside an allOf merge renders the
        // merge unsatisfiable--degrading to the never type--instead of
        // panicking partway through.
        let schema_json = r##"
        {
            "allOf": [
                { "$ref": "#/components/schemas/AppFeeAllocation" },
        ...
        let mut type_space = TypeSpace::default();
        type_space
            .add_ref_types([("WithDanglingRef".to_string(), schema)])
            .unwrap();
        let actual = type_space.to_stream().to_string();
        assert!(actual.contains("enum WithDanglingRef"), "{}", actual);
    }

    #[test]
    fn test_all_of_merged_metadata() {
        // Merging `true` with a metadata-carrying schema (or a $ref with a
        // schema it subsumes, as in the Meilisearch spec) yields the
        // surviving schema wholesale--metadata included. convert_all_of used
        // to assert that merged schemas carry no metadata; instead the outer
        // and merged metadata are now combined with outer values winning.
        let schema_json = r#"
        {
            "title": "OuterTitle",
            "allOf": [
        ...
        let _ = type_space.add_type(&schema.schema.into()).unwrap();
        let actual = type_space.to_stream().to_string();
        assert!(actual.contains("struct OuterTitle"), "{}", actual);
        assert!(actual.contains("inner description"), "{}", actual);
    }

    #[test]
    fn test_unknown_format_ignored_on_nonstring_types() {
        // Wild specs (Twilio, Cloudflare, Box) put format values like
        // "uri-map" on non-string types. An unrecognized format must be
        // ignored--converting the schema as if the format were absent--rather
        ...
        let type_id = type_space.add_type(&schema.schema.into()).unwrap();
        let ty = type_space.get_type(&type_id).unwrap();
        assert!(ty.ident().to_string().contains("HashMap"));

        // A format on a boolean is likewise annotation, not constraint.
        let schema_json = r#"{ "type": "boolean", "format": "checkbox" }"#;
        let schema: RootSchema = serde_json::from_str(schema_json).unwrap();
        let mut type_space = TypeSpace::default();
        let type_id = type_space.add_type(&schema.schema.into()).unwrap();
        let ty = type_space.get_type(&type_id).unwrap();
        assert_eq!(ty.ident().to_string(), "bool");
    }
