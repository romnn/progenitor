...
pub mod types {
    ...
    impl<'de> self::de::Build<'de> for UnoBody {
        const NAME: &'static str = "UnoBody";
        const FIELDS: &'static [&'static str] = &["gateway", "required"];
        fn build<E>(mut fields: self::de::Fields<'de>) -> ::std::result::Result<Self, E>
        where
            E: ::serde::de::Error,
        {
            let value = Self {
                gateway: self::de::defaulted(&mut fields, "gateway")?,
                required: self::de::required(&mut fields, "required")?,
            };
            ::std::result::Result::Ok(value)
        }
    }

    impl<'de> ::serde::Deserialize<'de> for UnoBody {
    ...
    impl ::serde::Serialize for UnoBody {
        fn serialize<S>(&self, serializer: S) -> ::std::result::Result<S::Ok, S::Error>
        where
            S: ::serde::Serializer,
        {
            let len = 0usize
                + ::std::primitive::usize::from(!::std::option::Option::is_none(&self.gateway))
                + 1;
            let mut state = ::serde::Serializer::serialize_struct(serializer, "UnoBody", len)?;
            if !::std::option::Option::is_none(&self.gateway) {
            ...
            ::serde::ser::SerializeStruct::serialize_field(&mut state, "required", &self.required)?;
